/**
* FontLoader 2.1 by Denis Kolyako. June 13, 2008. Last update: June 10, 2009.
...
package com.font {
	...
	public class FontLoader extends EventDispatcher {
		...
		public function FontLoader(request:URLRequest = null, autoRegister:Boolean = true) 
		{
			...
			if (request) 
				this.load(request, autoRegister);
		}
		...
		private function analyze(bytes:ByteArray, delayed:Boolean = false):void 
		{
			...
			if (this._fontCount) {
				...
				if (delayed) {
					this._libLoader = new Loader();
					this._libLoader.addEventListener(Event.ENTER_FRAME, this.handler_enterFrame);
				} else {
					this.close();
		...
		private function handler_enterFrame(event:Event):void {
			this._libLoader.removeEventListener(Event.ENTER_FRAME, this.handler_enterFrame);
			this.close();
			super.dispatchEvent(new Event(Event.COMPLETE));
		}
