...
package gFrameWork.display
{
	...
	public class BitMovieClip extends PivotSprite
	{
		/**
		 * 动画播放数据源 
		 */		
		 ...
		 * 是否在播放中 
		 */		
		private var m_playing:Boolean = true;
		...
		public function BitMovieClip(frame:BitmapDataAtlas,fps:int=30,loop:Boolean = true,reverse:Boolean=false):void
		{
			...
			if(m_bitmapDataAtlas)
			{
				...
				if(totalFrames > 0)
				{
					...
					//new JT_MovieClip 后必须手动调用Player才会播放动画，为了节省cpu
//					updateFrame();
				}
			}
		}
		...
		{
			addFrameAt(totalFrames,bitmapData,null,-1);
		}
		
		public function addFrameAt(frameID:int,bitmapData:BitmapData,sound:Sound,duration:Number=-1):void
		{
